...
pub trait Interceptor {
    fn invoke_func(&self, name: &String, executor: &Executor, store: &Store) -> ExecResult<Signal>;
...
impl Interceptor for NopInterceptor {
    fn invoke_func(&self, _name: &String, _executor: &Executor, _store: &Store) -> ExecResult<Signal> {
        Ok(Signal::Next)
    }
