...
pub fn instantiate_wasi() -> (WasiContext, HashMap<String, HostValue>) {
    ...
    fn define_wasi_fn<
        F: Fn(&[WasmValue], &mut Vec<WasmValue>, &mut HostContext, &mut WasiCtx) -> Result<(), Trap>
            + 'static,
    >(
    ...
    let func = define_wasi_fn(vec![ValueType::I32], None, |args, _ret, _ctx, _wasi_ctx| {
        unsafe {
    ...
    let func = define_wasi_fn(vec![], Some(ValueType::I32), |_args, ret, _ctx, _wasi_ctx| {
        unsafe {
